      ...
      function ($scope, $rootScope, appService, dateService, requestService, Stack, Workflow, MachineType) {
        ...
        function buildMachineDeprecationDataBasedOnGracePeriodDates(startDate, endDate) {
          ...
          if (currentDate < startDate) {
            return {
              message: `Upgrading to newer generation Apple Silicon machines, with no extra costs. M2 Pro X Large, M2 Pro Large or M1 Medium machines will be deprecated and your Workflows will automatically run on newer generation Apple Silicon machines from ${dateService.toLocaleMonthDayDateString(startDate)}.`,
          ...
          if (currentDate >= startDate && currentDate <= endDate) {
            return {
              message: `Your Workflows were automatically switched to newer generation Apple Silicon machines, with no extra costs. To continue using the deprecated M2 Pro X Large, M2 Pro Large or M1 Medium machines until the ${dateService.toLocaleMonthDayDateString(endDate)}, opt to extend the transition period in the`,
          ...
          if (currentDate > endDate) {
            return {
              message:
                'Your Workflows are now running on newer generation Apple Silicon machines, with no extra costs. If you’d like to keep using the deprecated M2 Pro X Large, M2 Pro Large or M1 Medium machines, reach out to support.',
              linkUrl: machineUpgradeAnnouncementUrl,
              linkLabel: 'Read announcement.',
            };
          }
        ...
        $scope.$on(
          ...
          $rootScope.$on('MainController::saveSuccess', (_event, { forceReload, menu }) => {
            const shouldReload = forceReload || menu !== 'stack';
