...
type UseStepBundleInstanceResult = Props & {
...
const useStepBundleInstance = (props: Props) => {
  const defaultStepLibrary = useDefaultStepLibrary();

  return useBitriseYmlStore(({ yml }) => {
    const { stepBundleId, parentWorkflowId, parentStepBundleId, stepIndex } = props;

    const result: UseStepBundleInstanceResult = { stepBundleId, parentWorkflowId, parentStepBundleId, stepIndex };
...
export default useStepBundleInstance;
