  ...
  describe('prepareStackAndMachineSelectionData', () => {
    ...
    it('returns the invalid stack when an invalid stack is selected', () => {
      ...
      expect(result.selectedMachineType).toEqual(
        expect.objectContaining({ value: 'mac-m1', id: 'mac-m1', name: 'mac-m1' }),
      );
      ...
      expect(result.machineOptionGroups).toEqual([
        {
          ...
          status: 'unknown',
          options: [{ value: 'mac-intel', label: 'mac-intel', os: 'unknown', status: 'unknown' }],
        },
        {
          label: 'Default Machine',
          status: 'available',
      ...
      expect(result.machineOptionGroups).toEqual([
        {
          ...
          status: 'available',
          options: [{ value: '', label: 'Self-Hosted Runner', status: 'available' }],
        },
      ]);
    });
      ...
      expect(result.machineOptionGroups).toEqual([
        {
          ...
          status: 'available',
          options: [{ value: '', label: 'Dedicated Machine', status: 'available' }],
        },
      ]);
    });
