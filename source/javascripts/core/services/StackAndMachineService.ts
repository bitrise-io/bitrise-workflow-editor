...
function prepareStackAndMachineSelectionData(props: SelectStackAndMachineProps): SelectStackAndMachineResult {
  ...
  const result: SelectStackAndMachineResult = {
    selectedStack: createStack({ id: selectedStackId, name: selectedStackId, value: selectedStackId }),
    ...
    result.machineOptionGroups = [
      {
        ...
        status: 'available',
        options: [{ label: 'Self-Hosted Runner', value: '', status: 'available' }],
      },
    ];
    return result;
  }
  ...
  const osMachineId = defaultMachineTypeIdOfOSs[result.selectedStack.os];
