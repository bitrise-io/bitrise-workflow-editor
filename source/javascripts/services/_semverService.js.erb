...
angular.module("BitriseWorkflowEditor").factory("semverService", ["stringService", function(stringService) {
    var semverService = {
        WILDCARD: WILDCARD,
        WILDCARD_REGEX: WILDCARD_REGEX,
        semver: function(verPayload) {
            return stringService.stringReplacedWithParameters("<major>.<minor>.<patch>", verPayload)
        }
    };
    ...
    semverService.checkVersionPartsLocked = function(version, lockedParts) {
        var wildcardPart = _.constant("." + WILDCARD);
        var expectedPostfix = _.times(lockedParts || 1, wildcardPart).join("");

        return version && version.endsWith(expectedPostfix);
    };
    ...
    semverService.resolveVersion = function(version, stepId, stepCatalogue) {
        var version = semverService.normalizeVersion(version);
        ...
        var stepVersions = Object.keys(stepCatalogue.steps[stepId]).sort(reverseSorter);
