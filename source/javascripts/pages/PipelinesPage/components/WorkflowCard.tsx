...
import { useAfterRunWorkflows, useBeforeRunWorkflows } from '@/hooks/domain/useWorkflowChain';
import { useWorkflowById } from '@/hooks/domain/useWorkflow';
import useMeta from '@/hooks/domain/useMeta';
...
const WorkflowCard = ({ id, isRoot = true, isExpanded = false }: WorkflowCardProps) => {
  ...
  const { title, meta: workflowMeta, steps } = useWorkflowById({ id });
