...
import { isUpgradeableStep, Step } from '@/models/Step';
...
const StepConfigDrawerContent = (props: UseDisclosureProps) => {
  ...
  const form = useForm<Step>({ mode: 'all', shouldUnregister: true });
  const [selectedTab, setSelectedTab] = useState<string | undefined>('configuration');
  const { isLoading, step, icon, title, resolvedVersion, maintainer, availableVersions } = useStepDrawerContext();

  const stepHasOutputVariables = (step?.outputs?.length ?? 0) > 0;

  useLayoutEffect(() => {
    if (isOpen && !isLoading && step && isEmpty(form.formState.defaultValues)) {
      form.reset(step);
    }
  }, [isOpen, isLoading, form, step, title]);
          ...
          >
            ...
            <DrawerFooter p="32" boxShadow="large">
              <ButtonGroup spacing={16}>
                <Button isDisabled={isEmpty(form.formState.dirtyFields)}>Done</Button>
