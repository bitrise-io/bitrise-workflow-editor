...
const StepVersionDetails = ({
  ...
  const stepVersionText = useMemo(() => {
    const version = step.isConfigured() ? resolvedVersion : "Invalid version";
    if (step.isVCSStep()) {
      return `Branch: ${version}`;
    }
    return `Version: ${version}`;
  }, [resolvedVersion, step]);
      ...
      <Box className="version-info">
        <div className="resolved-version">
          {hasVersionUpdate ? (
            <button
              type="button"
              aria-label="Update step version"
              data-e2e-tag="step-version-details__update-button"
              className="icon icon-danger"
              onClick={() => onChange({ version: "" })}
            >
              <img
                alt="Update step version icon"
