(function() {
	...
	angular
		...
		.factory("Popup", function($rootScope, $q) {
			...
			Popup.showConfirmPopup = function(
				title,
				details,
				yesTitle,
				noTitle,
	...
	angular
		.module("BitriseWorkflowEditor")
		.directive("popup", function($animate, $document, $window, $parse) {
			return {
				restrict: "E",
				replace: true,
				template: function(tElement, tAttrs) {
					return (
						"<div class='popup ng-hide angular-animated' ng-show='" +
						tAttrs.model +
						".isVisible'></div>"
					);
				},
				...
				link: function(scope, element, attrs, ctrl, transcludeFn) {
					...
					function scrollPreventHandler() {
						angular
							.element($window)
							.scrollTop(windowVerticalScrollPositionOnPopupOpen);
					}

					function setShouldHandleDismissTriggerEvents(
						shouldHandleDismissTriggerEvents
					) {
					...
					function setShouldPreventBackgroundScrolling(
						shouldPreventBackgroundScrolling
					) {
						if (shouldPreventBackgroundScrolling) {
							angular.element($window).bind("scroll", scrollPreventHandler);
							windowVerticalScrollPositionOnPopupOpen = angular
								.element($window)
								.scrollTop();
						} else {
								...
								if (newIsVisible) {
									...
									if (initiallyFocusedElement.length > 1) {
										initiallyFocusedElement.each(function() {
											if (
												$parse($(this).attr("initial-focus"))($(this).scope())
											) {
									...
									if (popup.afterAppearCallback) {
										$animate.on("removeClass", element, function(
											_element,
											phase
										) {
											if (phase == "start") {
