describe("WorkflowsSelectionService", () => {
	...
	describe("restoreSelection", () => {
		...
		it("should set edited workflow based on store check", () => {
			...
			expect(mockVm.editWorkflowAtIndex).toHaveBeenCalledWith(mockStore.lastEditedWorkflowIndex);
		});
		...
		it("should set selected step based on store check", () => {
			...
			mockVm.editedWorkflow = { steps: [{ id: "first-step" }, { id: "second-step" }] };

			selectionService.restoreSelection(mockVm);

			expect(mockVm.stepSelected).toHaveBeenCalledWith(mockVm.editedWorkflow.steps[1], undefined, true);
		});
